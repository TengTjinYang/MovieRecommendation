...
def generate_citation(tconst):
    ...
    movie_principals_rows = principals_df[principals_df['tconst'] == tconst]
    ...
    self_rows = movie_principals_rows[movie_principals_rows['category'] == 'self']
    if not self_rows.empty:
        # Get the nconst for 'self' entries
        for _, row in self_rows.iterrows():
            nconst_self = row['nconst']
